...
from cgnal.config import get_all_configuration_file, __this_dir__ as config_dir, merge_confs, \
    BaseConfig, FileSystemConfig, AuthConfig, AuthenticationServiceConfig
from cgnal.data.layer.mongo import MongoConfig
...
class TestLoggingConfig(TestCase):
    ...
    def test_default_config_file(self):
        self.assertEqual(config.logging.default_config_file, os.path.join("confs", "logConfDefaults.yaml"))
...
class TestAuthenticationServiceConfig(TestCase):
    ...
    def test_jwt_free_endpoints(self):
        self.assertEqual(config.authentication.jwt_free_endpoints, ['/api/v1/health/',
                                                                    '/api/v1/auth/login',
                                                                    '/api/v1/apidocs',
                                                                    '/api/v1/swagger.json',
                                                                    '/api/v1/salesforce/',
    ...
    def test_auth_service(self):
        ...
        self.assertEqual(config.authentication.auth_service.check, "/tokens/{tok}/check")
        self.assertEqual(config.authentication.auth_service.decode, "/tokens/{tok}/decode")

    @logTest
    def test_check_service(self):
        self.assertEqual(config.authentication.check_service.url, 'http://0.0.0.0:10001')
        self.assertEqual(config.authentication.check_service.login, '/authentication/login')
        self.assertEqual(config.authentication.check_service.logout, '/authentication/logout')
...
class TestMongoConfig(TestCase):
    ...
    def test_auth(self):
        ...
        self.assertEqual(config.mongo.auth.filename, os.path.join(root, "myfolder", "credentials.auth.p"))
    ...
    def test_admin(self):
        ...
        self.assertEqual(config.mongo.admin.filename, os.path.join(root, "myfolder", "credentials.admin.p"))
