...
from cgnal.data.model.core import LazyIterable, CachedIterable
...
class CachedDocuments(CachedIterable):
    ...
    def to_df(self, fields: List[str] = []) -> pd.DataFrame:
        return pd.DataFrame.from_dict({doc.uuid: {field: self.__get_key__(field, doc.data)
                                                  for field in fields}
                                       for doc in self.items}, orient='index')


class LazyDocuments(LazyIterable):
...
Documents = Union[LazyDocuments, CachedDocuments]
