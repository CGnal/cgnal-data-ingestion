...
class SeriesDAO(DAO[pd.Series, pd.Series]):
    ...
    def __init__(self, mapping: Optional[Dict[Hashable, Hashable]] = None, keys: Optional[Sequence[Hashable]] = None):
        """
        Data Access Object for pd.Series
    ...
    def inverseMapping(self) -> Dict[Hashable, Hashable]:
        """
        Inverse mapping between field names
    ...
    def get(self, serie: pd.Series) -> pd.Series:
        ...
        return row.append(pd.Series(self.computeKey(serie))) if self.keys is not None else row
